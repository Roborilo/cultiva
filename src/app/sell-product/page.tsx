...
export default function SellProductPage() {
  ...
  const HandlePost = async () => {
    ...
    if (
      !verifyStrings(payload.name) ||
      !verifyStrings(payload.description) ||
      !verifyStrings(payload.category)
    ) {
    ...
    if (response) {
      ...
      setAlertText(
        'Erro ao criar o produto, se você não cadastrou seu cpf ou cnpj, não será capaz de criar um novo cadastro',
      )
      setModal(true)
    }
    buttonRef.current?.removeAttribute('disabled')
