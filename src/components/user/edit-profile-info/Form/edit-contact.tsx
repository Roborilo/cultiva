...
export default function EditContact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const hasAddress = !!session?.user.addressId
    ...
    if (
      (!payload.telephone && payload.street) ||
      payload.number ||
      payload.district ||
      payload.cep ||
      payload.city ||
    ...
    if (!address && !payload.telephone) {
      setTextAlert(
        'Preencha todos os campos da área de endereço ou telefone para salvar',
      )
      setModal(true)
      return
    }

    const response = await useEditContactInfo(
      token,
      payload.telephone,
      address,
      hasAddress,
    ...
    if (response) {
      setTextAlert(
        'Informações alteradas com sucesso, recomendamos que faça login novamente para uma melhor experiência',
      )
      setModal(true)
    } else {
      setTextAlert(
        'Erro ao alterar as informações, tente novamente, por favor verifique se os dados foram inseridos corretamente',
      )
      setModal(true)
    }
  }
    ...
    <form className="flex flex-col gap-4 w-2/5" onSubmit={handleSubmit}>
      ...
      <div className="flex gap-4 w-full">
        ...
        <TextField.Input
          ...
          className="py-4 text-lg"
          onChange={(e) =>
            setPayload({ ...payload, number: Number(e.target.value) })
          }
          type="number"
        />
      </div>
      ...
      <div className="flex gap-4 w-full">
        <TextField.Input
          ...
          radius="small"
          className="py-4 text-lg"
          onChange={(e) => setPayload({ ...payload, cep: e.target.value })}
        />
        <TextField.Input
          placeholder="Estado"
          size={'3'}
      ...
      <button
        className="text-xl font-medium py-2 px-3.5 bg-green-600 hover:bg-green-700 rounded-xl w-4/12"
        type="submit"
      >
        Salvar
      </button>
