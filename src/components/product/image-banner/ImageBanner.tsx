...
export default function ImageBanner({ images }: { images?: string[] }) {
  const [actualImage, setActualImage] = useState(
    images
      ? images[0]
      : 'https://information-science-engineering.newhorizoncollegeofengineering.in/wp-content/uploads/2020/01/default-placeholder.png',
  )
    ...
    <>
      <div className="flex flex-col gap-4 h-[510px] w-32">
        {img.map((image, index) => (
          <div
            key={index}
            className="cursor-pointer hover:bg-opacity-80 w-28 max-h-28"
          >
            <img
              src={image}
              alt=""
