  ...
  return (
    ...
    >
      <AccordionItem value="item-1" label="Entrega">
        <DeliveryForm checked={checked} handleClick={() => setChecked(!checked)} payload={payload} setPayload={setPayload} />
      </AccordionItem>

      <AccordionItem value="item-2" label="Pagamento">
        <PaymentForm checked={checked2} handleClick={() => setChecked2(!checked2)} payload={payload2} setPayload={setPayload2} />
      </AccordionItem>
    </Accordion.Root>
  );
}
...
const AccordionItem = forwardRef(
  ({ value, label, children, className, ...props }: AccordionItemProps, forwardedRef) => (
    <Accordion.Item
      value={value}
...
const AccordionTrigger = forwardRef(
	({ children, className, ...props }: { children: React.ReactNode; className?: string }, forwardedRef) => (
