...
export default function PaymentForm({ checked, handleClick, payload, setPayload }: { checked: boolean, handleClick: () => void, payload?: Payload, setPayload?: React.Dispatch<React.SetStateAction<Payload | undefined>> }) {
	return (
		<div>
			<Form.Root className="w-full grid grid-cols-2 gap-6">
				<Form.Field className="grid mb-[10px]" name="email">
					...
					<Form.Control asChild>
						<input
							...
							type="text"
							required
							onChange={(e) => setPayload && setPayload({ ...payload, name: e.target.value })}
							value={payload?.name || ''}
						/>
					</Form.Control>
				</Form.Field>
				<Form.Field className="grid mb-[10px]" name="email">
					<div className="flex items-baseline justify-between">
						<Form.Label className="text-[15px] font-medium leading-[35px] text-white">Cartão</Form.Label>
						<Form.Message className="text-[13px] text-white opacity-[0.8]" match="valueMissing">
					...
					<Form.Control asChild>
						<input
							...
							type="number"
							required
							onChange={(e) => setPayload && setPayload({ ...payload, cardNumber: Number(e.target.value) })}
				...
				<Form.Field className="grid mb-[10px]" name="question">
					...
					<Form.Control asChild>
						...
						type="date"
						required
						onChange={(e) => setPayload && setPayload({ ...payload, validity: e.target.value })}
				...
				<Form.Field className="grid mb-[10px]" name="question">
					...
					<Form.Control asChild>
						...
						type="number"
						required
						onChange={(e) => setPayload && setPayload({ ...payload, cvc: Number(e.target.value) })}
						value={payload?.cvc || ''}
					/>
					</Form.Control>
				</Form.Field>
