...
export default function SidebarUser({
  ...
  useEffect(() => {
    const rate =
      (user &&
        user.rating &&
        user.rating.reduce((acc, curr) => acc + curr, 0) /
          user.rating.length) ||
      0
    ...
    <SidebarRoot>
      ...
      <div className="flex flex-col justify-center items-start gap-8 mt-3 w-full px-5 py-2">
        {status === 'authenticated' && session?.user?.id === pathId && (
          <>
            ...
            <div className="flex flex-col gap-4 px-4 w-full">
              <div
                className={`flex gap-4 items-center justify-start rounded-lg p-3 cursor-pointer w-full, ${
                  page === 'profile' && 'bg-green-700'
                }`}
        ...
        <div className="flex flex-col gap-4 px-4 w-full">
          ...
          <div
            className={`flex gap-4 items-center justify-start rounded-lg p-3 cursor-pointer w-full, ${
              page === 'sells' && 'bg-green-700'
            }`}
            onClick={() => setPage('sells')}
          >
          ...
          <div
            className={`flex gap-4 items-center justify-start rounded-lg p-3 cursor-pointer w-full, ${
              page === 'buys' && 'bg-green-700'
            }`}
            onClick={() => setPage('buys')}
          >
