  ...
  return (
    <SidebarRoot>
      ...
      <div className="flex items-center justify-between gap-4 p-4 mx-6 border-b-2 border-white">
        <Link
          href={'/'}
          className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 border border-gray-700 rounded-sm"
          onClick={() => products.map((product) => dec(product.id))}
        >
          Cancelar
        </Link>
        <button
          className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 border border-green-700 rounded-sm"
